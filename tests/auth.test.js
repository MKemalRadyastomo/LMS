...
beforeAll(async () => {
  ...
  await global.__DB__.query("DELETE FROM users WHERE email LIKE $1", [
    "%@example.com",
  ]);
...
describe("Auth API", () => {
  ...
  describe("POST /v1/auth/register", () => {
    ...
    afterEach(async () => {
      ...
      await global.__DB__.query("DELETE FROM users WHERE email = $1", [
        newUser.email,
      ]);
    });

    it("should register a new user", async () => {
      const res = await request(server).post("/v1/auth/register").send(newUser);
    ...
    it("should not allow duplicate email registration", async () => {
      ...
      await request(server).post("/v1/auth/register").send(newUser);
